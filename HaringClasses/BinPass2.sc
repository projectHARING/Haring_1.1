	...
	*new { |frames = 512, id = 0|
	...
	init { |frames_, id_|
		...
		if(server.serverRunning) {
			...
			bands = this.getBands;
			instrumentName = (\Haring_binPassFilter_++id).asSymbol;
			reverbName =(\Haring_reverb_++id).asSymbol;
			this.declareSynths;
	...
	morphInspector{ |winPosX = 0, winPosY = 0, masterWin|
		...
		inspectorWindow = styler.getWindow("test", Rect(winPosX, winPosY, (numBands * 2) + 38, winHeight));
		...
		multiInspect = MultiSliderView(inspectorWindow, Rect(0, 0, (numBands * 2) + 23, winHeight * 0.75))
		.elasticMode_(0)
		.value_(zeroVals)
		.isFilled_(true)
		.indexThumbSize_(3)
